...
protected:
	...
	const float* GetCascadeSplits(const float& _near, const float& _far, const uint8_t& _cascadeCount, const float& _lambda) const;
	const glm::vec3* GetFrustumCorners(const glm::mat4& _viewProjection) const;
	const glm::mat4* GetCascadeProjections(const glm::mat4 _cameraProj, const glm::mat4& _cameraView, const glm::mat4& _cameraViewproj, const uint32_t& _cascadeCount, const float& _near, const float& _far, const glm::vec3& _lightDir) const;
