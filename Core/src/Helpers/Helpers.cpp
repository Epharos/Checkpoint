...
QJsonObject Helper::Serialize::SerializeVector3(const glm::vec3& _vector)
{
	QJsonObject data;
...
QJsonObject Helper::Serialize::SerializeQuaternion(const glm::quat& _quaternion)
{
	QJsonObject data;

	data["x"] = _quaternion.x;
	data["y"] = _quaternion.y;
	data["z"] = _quaternion.z;
	data["w"] = _quaternion.w;

	return data;
}

glm::vec3 Helper::Serialize::DeserializeVector3(const QJsonObject& _json)
{
	glm::vec3 vector;

	vector.x = _json["x"].toDouble();
	vector.y = _json["y"].toDouble();
	vector.z = _json["z"].toDouble();

	return vector;
}

glm::quat Helper::Serialize::DeserializeQuaternion(const QJsonObject& _json)
{
	glm::quat quaternion;

	quaternion.x = _json["x"].toDouble();
	quaternion.y = _json["y"].toDouble();
	quaternion.z = _json["z"].toDouble();
	quaternion.w = _json["w"].toDouble();
